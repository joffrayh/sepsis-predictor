...
def sample_and_hold(df, vitalslab_hold):
    """
    Forward-fill clinical measurements per ICU stay up to per-concept hold time.
    ...
    df : pd.DataFrame
        ...
        guaranteed by ``process_patient_measurements``. Columns are
        concept names.
    vitalslab_hold : dict[str, int]
        ...
        processed. Source: ``hold_times`` returned by
    ...
    Notes
    -----
    Two ``groupby(...).transform("ffill")`` passes are used: one to
    propagate the timestamp of the last valid reading, one to propagate
    its value. A gap is filled only when the current row is NaN **and**
    the elapsed time since the last valid reading is within the hold
    period. Gaps exceeding the hold period remain NaN for KNN to handle
    downstream.
    """
    ...
    for col in tqdm(cols_to_process, desc="\tProcessing columns", ncols=100):
        ...
        df.loc[gap_mask & within_hold, col] = last_valid_value[
            gap_mask & within_hold
        ]
...
def fixgaps(x: np.ndarray) -> np.ndarray:
    ...
    y[nan_mask] = interp1d(valid_indices, x[valid_indices])(
...
def handle_missing_values(df, missing_threshold=0.8, knn_neighbors=1):
    ...
    1. **Drop** — columns with global missingness \u2265
       ``missing_threshold`` are removed entirely.
    ...
    3. **KNN** — ``KNNImputer`` on all remaining measurement columns with
       missingness \u2265 5%. Patients are grouped into chunks of ~9,999
       rows (whole patients only) so KNN never imputes across patient
       boundaries.
    ...
    missing_threshold : float, optional
        ...
        Columns at exactly this value are kept (comparison is
        ``< missing_threshold``).
    ...
    Patient-chunk boundaries are determined by
    ``groupby("stay_id", sort=False)``
    (insertion order, not ``stay_id`` sort order) to avoid a redundant
    re-sort.
    """
    ...
    measurement_cols = [
        col for col in df.columns if col not in non_measurement_cols
    ]

    miss = df[measurement_cols].isna().sum() / len(df)
    cols_to_keep = miss[miss < missing_threshold].index
    df = df[
        df.columns[~df.columns.isin(measurement_cols)].tolist()
        + cols_to_keep.tolist()
    ]
    ...
    if cols_for_knn:
        ...
        print(
            f"\tRunning KNN imputation across {len(chunks)}"
            " patient-aligned chunks..."
        )
        for chunk_indices in tqdm(chunks, desc="\tKNN imputation", ncols=100):
            imputer = KNNImputer(
                n_neighbors=knn_neighbors, keep_empty_features=True
            )
...
def add_missingness_features(df, lab_cols=None, timestep_hours=4):
    ...
    For each lab in ``lab_cols`` (if the column is present), two columns
    are appended:

    - ``<lab>_measured`` \u2014 float 0/1: 1 if the lab had a real
      value at this timestep.
    - ``hours_since_<lab>`` \u2014 float: hours since the last real
      measurement within the same patient. Pre-first-measurement
      rows receive 999.0 (sentinel meaning "never yet measured").
    ...
    Notes
    -----
    **Order-critical**: must be called after
    ``standardise_patient_trajectories``
    ...
    from the current index is multiplied by ``timestep_hours``. The
    999.0 sentinel is deliberate \u2014 not NaN \u2014 so downstream models
    treat it as "very long ago" without special handling.
    """
    ...
    for col in lab_cols:
        ...
        # Forward-fill the timestep index of the last valid reading
        # within each patient
        last_measured_step = df.groupby("stay_id")[
            last_measured_step.name
        ].ffill()
