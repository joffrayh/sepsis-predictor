...
def build_and_save_cohorts(config, path_config):
    ...
    for key, filename in files.items():
        data[key] = pd.read_csv(
            os.path.join(input_dir, filename), sep="|", low_memory=False
        )

    print("Loading and combining lab data...")
    labs_ce = pd.read_csv(
        os.path.join(input_dir, "labs_ce.csv"), sep="|", low_memory=False
    )
    labs_le = pd.read_csv(
        os.path.join(input_dir, "labs_le.csv"), sep="|", low_memory=False
    )
