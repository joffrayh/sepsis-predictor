...
def load_and_prepare_data(path, target_horizon=1, target_window=3):
    """
    load trajectory data and assemble sequence targets.
    deep learning models construct internal representations of time natively,
    so we do not explicitly engineer rolling features here like we do for trees.
    """
    ...
    # force temporal order to prevent sequence corruption
...
class SepsisLSTM(nn.Module):
    def __init__(self, input_dim, hidden_dim=64, num_layers=2, dropout=0.2):
        ...
        self.attention = nn.MultiheadAttention(embed_dim=hidden_dim, num_heads=4, batch_first=True, dropout=dropout)
...
def evaluate_model(y_true, y_probs, name="Model", threshold=None):
    ...
    print(f"Recall @ Threshold {threshold:.4f}: {recall:.4f}")
    
    return {'auprc': auprc, 'auroc': auroc, 'recall': recall, 'threshold': threshold}


def main():
    mlflow.set_experiment("Sepsis_Prediction_LSTM")
    
    # maximize intra-op threading specifically for data loading parallelization
    ...
    with mlflow.start_run():
        ...
        # natively scale physiological measurements since lstms suffer from gradient instability
        # if input variance differs astronomically between features
        ...
        batch_size = 256
        # cap background io workers to prevent memory blowout
        ...
        model = SepsisLSTM(input_dim=len(features), hidden_dim=64, num_layers=2).to(device)
        
        # extract positive occurrence ratio to drastically bump minority (sepsis onset) backpropagation weight
        ...
        for epoch in tqdm(range(epochs)):
            ...
            for X_b, y_b, mask_b in train_loader:
                ...
                # execute backwards pass strictly over real timesteps, effectively blinding the loss function to padded arrays
                loss = (loss * mask_b).sum() / mask_b.sum()
                
                loss.backward()
                # artificially cap huge gradient spikes to stop the lstm memory cells corrupting
            ...
            with torch.no_grad():
                for X_b, y_b, mask_b in val_loader:
                    ...
                    # slice out only unpadded probability rows for accurate val scoring
        ...
        with torch.no_grad():
            for X_b, y_b, mask_b in test_loader:
                ...
                # drop sequence buffer (zeros) completely before appending test outcomes
                valid_idx = mask_b.bool()
        ...
        mlflow.log_artifact("best_lstm.pth")
        
        # render calibration tracking predicted probabilistic spread against true incidence
        ...
        plt.legend()
        calib_path = os.path.join(ARTIFACT_DIR, "calibration_curve_v3.png")
