...
def main():
    ...
    # build the full cohort by merging onset times with all ICU stays, using intime as the anchor
    cohort = build_full_cohort(onset, demog)  # <-- add here

    # save processed data if requested
    if args.save_intermediate:
        print('Saving processed files...')
        cohort.to_csv('processed_files/cohort.csv', sep='|', index=False)
