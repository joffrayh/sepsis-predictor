...
# alchemy database connection string
db_url = f"postgresql+psycopg2://{auth_string}@localhost/mimiciv"

# alchemy engine to manage database connection and queries  
...
# run the queries inside a single connection
# we use a TEMP TABLE so nothing is saved to the DB after the script finishes
with engine.connect() as conn:
    
    print("Generating temporary Charlson Comorbidity Index...")
    
    charlson_query = """
    CREATE TEMP TABLE charlson_temp AS
    WITH diag AS (
        SELECT
            hadm_id
    ...
    conn.execute(text(charlson_query))
    
    print("Extracting Demographics and Sepsis Cohort...")
    ...
    FROM mimiciv_hosp.admissions ad
    INNER JOIN mimiciv_icu.icustays i ON ad.hadm_id = i.hadm_id
    INNER JOIN mimiciv_hosp.patients p ON p.subject_id = i.subject_id
    LEFT JOIN charlson_temp com ON ad.hadm_id = com.hadm_id -- Joining the TEMP TABLE
    ...
    d.to_csv(output_file, index=False, sep='|')

print(f"Success! Data saved to: {output_file}")
print(f"Rows extracted: {len(d)}")